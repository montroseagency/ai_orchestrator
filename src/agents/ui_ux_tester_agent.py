"""
UI/UX Tester Agent — Frontend adversarial quality gate.
Specializes in design system compliance, TypeScript, accessibility, and interaction quality.
Skills injected at call time: web_accessibility (always) + playwright_testing (conditional).
"""
...
class UIUXTesterAgent(BaseAgent):
    """
    Frontend-only adversarial tester.
    Checks design system compliance, TypeScript, accessibility, and interaction quality.
    Receives web_accessibility skill (always) and playwright skill (when server URL is set).
    """

    def __init__(self):
        super().__init__(
            name="UI/UX Tester",
            system_prompt_path=AGENTS_DIR / "ui_ux_tester.md",
            model=Config.UIUX_TESTER_MODEL,
        )
    ...
    async def test(
        ...
        """
        Test the frontend implementation and return a verdict.

        Args:
            context_package: Assembled context with implementation files + plan + design_brief
            session_id: Current session ID
            retry_count: How many test rounds have happened
            extra_system: Injected skills (web_accessibility + optional playwright)
        ...
        user_message = (
            ...
            f"Produce a complete UI/UX test report following the exact format in your system prompt.\n"
    ...
    async def write_walkthrough(
        ...
        user_message = (
            ...
            f"3. Notes any important architectural or design decisions made\n"
